  ...
  <header>
    ...
    <?php
      if(isset($_POST['btnElim'])){		
        ...
        $codi=$_POST['barra'];
        $resultado = mysqli_query($conectar,"SELECT * FROM productos WHERE  cod_barras = '$codi'");
        ...
        if (isset($_POST['btnAcep'])){
          include("abrirCon.php");
          $codi=$_POST['barra'];
          $sqlBorrar =  "DELETE FROM productos WHERE cod_barras = '$codi'";
          mysqli_query($conectar,$sqlBorrar);
          $cod_barras="";
		  $nombre_prod="";
          $precio_prod ="";
          $especialidad="";
	...
	<center>
        ...
        <table border="2px">
			...
			<form  class="form" method="POST" action="elimprod.php" method="post">
		...
		<p class="IbI_especialidad"> Carrera: </p><input type="text" id="epecial" name="epecial" value="<?php echo $especialidad;?>" placeholder="carrera del producto" class="txt_especialidad" required=true pattern="[A-Za-z]+" title="En este campo solo se aceptan letras">	
				...
				<input type="submit"  class="btn_registrar"  id="btnAcep" name="btnAcep" value="Acepto Eliminar" onclick="location.href=../HTML/prodElim.html"/>
